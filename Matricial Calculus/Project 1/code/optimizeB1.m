function [sells_opt, e_opt, u_opt, eps_opt, sig_opt] = optimizeB1(n_d, n_dof, n_el, n_nod, n_el_dof, Tn, Td, mat, Tmat, x, Fdata, Fext, fixNod)
%--------------------------------------------------------------------------
...
sells_opt = 0; % Optimal sells 
e_opt = 0; % Vector with optimal element bars
u_opt = 0; % Vector ith optimal displacement of each bar
...
for i = 0:(2^modBars-1)    
    ...
    if canSolve == 1        
        ...
        if sells_opt < sells
            ...
            e_opt = e_case;
            u_opt = u;
            [eps_opt, sig_opt] = computeStrainStressBar(n_d, n_el_case, u, Td, x, Tn_case, mat, Tmat_case);
        end
    end
end
